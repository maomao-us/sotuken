...
def kentei_train():
    for a in range(8):
        print(name[a])
        file_name1 = "sotuken_data_train/data/" + name[a] + "/" + name[a] + "_train_ev_" + lx[3] + ".csv"
        file_name2 = "sotuken_data_train/step/" + name[a] + "/" + name[a] + "_train_step_" + lx[3] + ".csv"
        ...
        with open("train_data/" + name[a] + "/" + name[a] + "_train_ev_" + lx[3] + ".csv", "w", newline = "") as f:
            writer = csv.writer(f)
            writer.writerow(mean_ev)
            writer.writerow(var_ev)
        with open("train_data/" + name[a] + "/" + name[a] + "_train_step_" + lx[3] + ".csv", "w", newline = "") as f:
            writer = csv.writer(f)
