...
def MIN_EV(c, d, e, f):
    ...
    # 前後20個のデータを格納
    for i in range(180, len(data) - sampling_number2):
    ...
    for z in range(3, 4):
        ...
        for i in range(10, 30):
            ...
            if csv_counter == 0 and len(ev) > 1:
                with open("sotuken_data/" + name[w] + "/" + name[w] + "_ev" + labs[z] + ".csv", "w", newline = "") as f:
                    writer = csv.writer(f)
                    writer.writerow(ev)
                with open("step/" + name[w] + "/" + name[w] + "_step" + labs[z] + ".csv", "w", newline = "") as f:
                    writer = csv.writer(f)
                    writer.writerow(ev_step)
                csv_counter += 1
            elif csv_counter >= 1 and len(ev) > 1:
                with open("sotuken_data/" + name[w] + "/" + name[w] + "_ev" + labs[z] + ".csv", "a", newline = "") as f:
                    writer = csv.writer(f)
                    writer.writerow(ev)
                with open("step/" + name[w] + "/" + name[w] + "_step" + labs[z] + ".csv", "a", newline = "") as f:
                    writer = csv.writer(f)
                    writer.writerow(ev_step)
                csv_counter += 1
