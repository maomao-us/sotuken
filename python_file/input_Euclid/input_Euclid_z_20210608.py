        ...
        for i in range(10, 30):
            ...
            if len(ev) == 2 * (z + 2):
                if i % 2 == 0 and count_train == 0:
                    with open("sotuken_data_train/data/" + name[w] + "/" + name[w] + "_train_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_train/step/" + name[w] + "/" + name[w] + "_train_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
                    count_train += 1
                
                elif i % 2 == 0 and 0 < count_train:
                    with open("sotuken_data_train/data/" + name[w] + "/" + name[w] + "_train_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_train/step/" + name[w] + "/" + name[w] + "_train_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
                
                elif i % 2 == 1 and count_test == 0:
                    with open("sotuken_data_test/data/" + name[w] + "/" + name[w] + "_test_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_test/step/" + name[w] + "/" + name[w] + "_test_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
                    count_test += 1
                
                elif i % 2 == 1 and 0 < count_test:
                    with open("sotuken_data_test/data/" + name[w] + "/" + name[w] + "_test_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_test/step/" + name[w] + "/" + name[w] + "_test_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
